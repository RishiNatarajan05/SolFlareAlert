...
Keep good performance while fixing overconfidence with minimal changes
"""
...
def balanced_calibration():
    """Balanced calibration approach"""
    print("=== Balanced Calibration Approach ===\n")
    ...
    # 2. Balanced parameters - keep good performance, add minimal calibration
    print(f"\n2. Using balanced parameters...")
    
    # Keep original scale_pos_weight but add max_delta_step
    scale_pos_weight = 6.2  # Keep original
    ...
    params = {
        ...
        'min_child_weight': 3,
        'gamma': 0.1,  # Keep original
        'scale_pos_weight': scale_pos_weight,  # Keep original
        'base_score': 0.15,  # Keep original
        'max_delta_step': 2,  # NEW: Moderate stabilization
        'reg_lambda': 1.0,  # NEW: Moderate L2 regularization
        'reg_alpha': 0.0,  # NEW: No L1 regularization
        'random_state': 42
    }
    
    print(f"Scale pos weight: {scale_pos_weight:.1f} (kept original)")
    print(f"Base score: {params['base_score']} (kept original)")
    print(f"Max delta step: {params['max_delta_step']} (NEW)")
    print(f"Reg lambda: {params['reg_lambda']} (NEW)")
    print(f"Gamma: {params['gamma']} (kept original)")
    
    # 3. Train model with balanced parameters
    print(f"\n3. Training model with balanced parameters...")
    ...
    # 7. Test thresholds for precision optimization
    ...
    # 9. Save balanced model
    print(f"\n9. Saving balanced model...")
    
    # Create metadata
    metadata = {
        'model_type': 'flare_prediction_balanced',
    ...
    model_path = f'models/flare_balanced_{timestamp}.pkl'
    metadata_path = f'models/flare_balanced_{timestamp}_metadata.json'
    ...
    print(f"Original AUC: 0.636")
    print(f"Balanced AUC: {auc_cal:.3f}")
    print(f"AUC Difference: {auc_cal - 0.636:.3f}")
    print(f"Original Precision: 0.179")
    print(f"Balanced Precision: {best_precision:.3f}")
    print(f"Precision Difference: {best_precision - 0.179:.3f}")
    print(f"Original FPR: 0.736")
    print(f"Balanced FPR: {best_fpr:.3f}")
    ...
    if best_precision > 0.15:
        print("✅ GOOD: Precision maintained!")
    else:
        print("⚠️  Precision decreased")
    
    if best_fpr < 0.8:
        print("✅ GOOD: False positive rate reasonable!")
    else:
        print("⚠️  False positive rate still high")
    
    print(f"\n=== Balanced Approach Summary ===")
    print(f"1. Kept original scale_pos_weight: {scale_pos_weight}")
    print(f"2. Kept original base_score: {params['base_score']}")
    print(f"3. Added moderate max_delta_step: {params['max_delta_step']}")
    print(f"4. Added moderate reg_lambda: {params['reg_lambda']}")
    print(f"5. Kept original gamma: {params['gamma']}")
