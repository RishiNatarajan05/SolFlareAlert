#!/usr/bin/env python3
"""
Test original working model without calibration
See if raw XGBoost probabilities are better distributed
"""
...
def test_original_model_no_calibration():
    """Test original model without calibration"""
    print("=== Original Model Without Calibration ===\n")
    
    # 1. Prepare data (same as working model)
    ...
    # 3. Train model WITHOUT calibration
    print(f"\n3. Training model WITHOUT calibration...")
    ...
    # Create metadata
    metadata = {
        'model_type': 'flare_prediction_no_calibration',
    ...
    model_path = f'models/flare_no_calibration_{timestamp}.pkl'
    metadata_path = f'models/flare_no_calibration_{timestamp}_metadata.json'
    ...
    print(f"\n=== No Calibration Model Complete ===")
    print(f"Key results:")
    print(f"1. AUC: {auc:.3f}")
    ...
    # Compare with calibrated model
    print(f"\n=== Comparison with Calibrated Model ===")
    print(f"Calibrated AUC: 0.508")
    print(f"Raw AUC: {auc:.3f}")
    print(f"AUC Difference: {auc - 0.508:.3f}")
    
    if auc > 0.508:
        print("✅ BETTER: Raw model performs better!")
    else:
        print("⚠️  WORSE: Calibrated model was better")

if __name__ == "__main__":
    test_original_model_no_calibration()
