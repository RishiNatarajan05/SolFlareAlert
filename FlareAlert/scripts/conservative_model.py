#!/usr/bin/env python3
"""
Train a very conservative model to minimize false positives
Use much more conservative parameters and focus on precision
"""
...
def train_conservative_model():
    ...
    # 2. Use very conservative XGBoost parameters
    print(f"\n2. Training with very conservative parameters...")
    
    # Use very low scale_pos_weight to make model conservative
    positive_samples = y_train.sum()
    scale_pos_weight = 1.5  # Much lower than before
    ...
    params = {
        ...
        'min_child_weight': 10, # Very high to reduce false positives
        'gamma': 0.5,           # Very high to reduce false positives
        'subsample': 0.6,       # Very low to reduce overfitting
        'colsample_bytree': 0.6, # Very low to reduce overfitting
        'reg_lambda': 5.0,      # Very high L2 regularization
        'reg_alpha': 1.0,       # High L1 regularization
        'n_estimators': 500,    # Fewer trees
        'max_delta_step': 5,    # Very high to stabilize probabilities
    ...
    print(f"\n5. Evaluating conservative model...")
    ...
    # 6. Optimize threshold for precision
    ...
    for threshold in thresholds:
        ...
        if precision > best_precision and recall > 0.05:  # Must have at least 5% recall
    ...
    # 8. Save conservative model
    print(f"\n8. Saving conservative model...")
    
    # Create metadata
    metadata = {
        'model_type': 'flare_prediction_conservative',
    ...
    model_path = f'models/flare_conservative_{timestamp}.pkl'
    metadata_path = f'models/flare_conservative_{timestamp}_metadata.json'
