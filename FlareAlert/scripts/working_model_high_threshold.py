#!/usr/bin/env python3
"""
Use working model with high threshold to reduce false positives
Keep the good AUC but use higher threshold for precision
"""
...
def working_model_high_threshold():
    """Use working model with high threshold"""
    print("=== Working Model with High Threshold ===\n")
    
    # Load the restored working model
    print("1. Loading restored working model...")
    with open('models/flare_restored_20250813_104100.pkl', 'rb') as f:
        model = pickle.load(f)
    ...
    # 4. Test high thresholds to reduce false positives
    print(f"\n4. Testing high thresholds...")
    
    # Test thresholds from 0.7 to 0.99
    ...
    # Create metadata
    metadata = {
        'model_type': 'flare_prediction_working_high_threshold',
    ...
    metadata_path = f'models/flare_working_high_threshold_{timestamp}_metadata.json'
    ...
    print(f"\n=== Working Model with High Threshold Complete ===")
    print(f"Key results:")
    print(f"1. AUC: {auc:.3f} (maintained good performance)")
    ...
    print(f"7. Positive Predictions: {best_predictions}")
    ...
    print(f"- AUC maintained at: {auc:.3f}")

if __name__ == "__main__":
    working_model_high_threshold()
